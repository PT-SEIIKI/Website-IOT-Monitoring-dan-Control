...
export function Sidebar({ onNavClick, isMobile = false }: SidebarProps) {
  ...
  return (
    <aside 
      ...
      className={cn(
        "flex flex-col h-[calc(100vh-1.25rem)] m-1.5 md:m-2 lg:m-2.5 rounded-xl bg-sidebar border border-sidebar-border transition-all duration-300 relative z-40 shadow-lg",
        sidebarWidth
      )}
    >
      {/* Logo */}
      <div className="flex items-center gap-2 p-3 border-b border-sidebar-border">
        ...
        {!collapsed && (
          <div className="animate-fade-in truncate">
            ...
            <p className="text-[10px] text-muted-foreground">Campus System</p>
          </div>
        )}
      </div>
        ...
        {filteredNav.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span className={cn(
                "font-medium text-xs animate-fade-in truncate transition-opacity duration-300",
      ...
      <div className="p-2 border-t border-sidebar-border">
        {!collapsed && user && (
          <div className="px-2 py-1.5 mb-1.5 rounded-lg bg-muted/50 animate-fade-in overflow-hidden text-left">
        ...
        >
          ...
          {!collapsed && <span className="font-medium text-xs animate-fade-in">Logout</span>}
        </button>
      </div>
